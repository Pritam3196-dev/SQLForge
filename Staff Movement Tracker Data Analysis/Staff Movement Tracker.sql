...
select staff_id , last_name , Email_id , Gender , department , start_date , salary , job_title  , region_id ,
...
select * from staff as s inner join company_divisions as cd on s.department = cd.department;


-- 21.now all 1000 staffs are returned, but some 47 people have missing company - division.


select * from staff as s left join company_divisions as cd on s.department = cd.department;


-- 22.who are those people with missing company division? (Those people which are company_division null)

select * from staff as s left join company_divisions as cd on s.department = cd.department 
where cd.company_divsion is null;
...
select country , company_regions , count(staff_id) as No_of_Employees , count(last_name) as No_of_staff , 
Row_number() over(partition by country order by count(staff_id) desc) as Rank_company_regions
from staff as s inner join company_regions as cr on s.region_id = cr.region_id group by country , company_regions;



-- 25.number of employees per regions & country, Then sub totals per Country, Then toal for whole table

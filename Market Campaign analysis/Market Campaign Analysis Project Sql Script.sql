-- We create Data Analytics project based on Market Campaign analysis


--- Creating Database


create database Market_Campaign_Analysis;
...
create table Item(
			   ...
			   Item_Category varchar(228)  not null,
			   Price Int not null
);


-- create couponmapping table
...
------- Creating CustomerTransactionData Table
...
Insert into Item(Item_Id , Item_Name ,Item_Category ,Price) values
...
----------- Inserting data into Couponmapping table -------------------------

insert into Couponmapping (Coupon_Id ,Item_Id , CouponType,value ,Min_Purchase , Expiry) values
...
Insert into Campaign(Campaign_Id , CampaignType ,start_Date , end_Date) values
...
Insert into City(City_Id , CityName , State , CityTier) values
(1, 'Delhi', 'Delhi', 1),
...
Insert into Customer(Customer_Id , name , gender , City_Id , Pincode ,BirthDate, Income_bracket,EmailId , PhoneNo)
values
...
----------- ----------- Inserting data into CustomerTransactiondata table -------------------------



Insert into CustomerTransactionData (Trans_id ,Cust_Id ,Item_id,Campaign_Id , Coupon_id, quantity ,purchasingAmt ,
PurchaseDate , OrderType)
values
...
------------------ In section first we are understanding of data intrems of numbers --------------------------

-- 1. Different type of paints provided by the company.

select count(distinct(Item_category)) as count_of_paints_type from  Item;
...
select distinct State from city as c 
inner join customer as cu 
on c.city_Id = cu.city_Id
inner join CustomerTransactionData as ct
...
----Question 2 is overview regarding Transactions in terms of numbers

--1. Yearly basis

select DatePart(Year , PurchaseDate ) as Transaction_Year , count(Trans_Id)  as Count_Transaction
from CustomerTransactionData group by DatePart(Year , PurchaseDate ) 
...
select DatePart(quarter , PurchaseDate) as quarter ,count(Trans_Id) as Transaction_Count
from CustomerTransactionData group by DatePart(quarter , PurchaseDate) 
...
select Datepart(Year , PurchaseDate) as Year , Datepart(Month , PurchaseDate) as Month ,
count(Trans_Id) as Count_Transaction from CustomerTransactionData
...
----- Question 3 is regarding regarding total purchase  menans it is regarding orders amount

--1. Product category
-- purchased product category means those product category which get purchased and it ask 
-- total amount of sales by item category

select Item_category , sum(PurchasingAmt) as Total_sales from Item as I
inner join CustomerTransactionData as ct
...
--- It asks total sales product category on basis of yearly and Quarterly basis 


select Datepart(Year , PurchaseDate) as Year , Datepart(quarter , purchaseDate) as quarter  ,
sum(PurchasingAmt) as Total_sale
from item as i
inner join CustomerTransactionData as ct
on i.Item_id = ct.Item_id
...
select orderType ,sum(PurchasingAmt) as Total_Purchase  
from CustomerTransactionData group by OrderType order by Total_Purchase desc; 

/*

Household	4190980.85
Government	3929597.6
Industrial	2725429.65 
...
select cityTier  , round(sum(PurchasingAmt) , 2) as Total_sale from city as c 
inner join customer as cu
on c.city_id = cu.city_id
inner join CustomerTransactionData as ct
on cu.customer_id = ct.Cust_id
...
select count(Trans_id) from CustomerTransactionData;

select 'purchase without coupon' as 'Coupon_status' , count(Trans_id) as No_of_transactions from CustomerTransactionData 
where coupon_id is not null
union all
select 'purchase with coupon' as 'Coupon_status' , count(Trans_id) as No_of_transactions from CustomerTransactionData
where coupon_id is NULL


-- 2. Identify the number of customers with first purchase done with or without campaign coupons.

select 'Customers with coupons'  , count(distinct(cust_id)) from CustomerTransactionData where Coupon_id is null;

select 'Customers without coupons' , count(distinct(cust_id)) from CustomerTransactionData where Coupon_id is not null;



-- -- 2. identify the name of potential leads who have made a purchase but have not yet used a campaign coupon 

------ potential lead means customer who have coupon but they not used..... Here we we talking about those people 
--- who purchase but coupon is not used ...
--- 1. To get customers who purchase ........  the customer who have trans_id

--- find those customers who have trans_id and then check is there coupon is null
---- if name wants then join

select ct.cust_id , name , count(trans_id) as no_of_Purchases from customer as c 
inner join CustomerTransactionData as ct
on c.customer_id = ct.Cust_Id
where coupon_id is null
group by ct.cust_id , name;

-- the customer have trans_id those customers are purchase customers and check where coupon_id is null or not ?


--- if name does not want then use this 
select Cust_id , count(Trans_id) as no_of_Purchases from CustomerTransactionData
where coupon_id is null
group by cust_id;
...
--The marketing team is interested in understanding the growth and decline pattern of the company in terms 
--of new leads or sales amount by the customers.
-- 1. Identify the total growth on an year by year basis excluding the current year
-- a. Based on quantity of paint that is sold

select  Year(purchaseDate)as Year , Item_category ,sum(quantity) as Quantity_Items , 
sum(PurchasingAmt) as Total_purchase
from CustomerTransactionData as c
inner join Item as I
on c.Item_id = I.Item_id
where Year(purchaseDate) != Year(GetDate())
group by Year(purchaseDate) , Item_category ; 

---- ## 3.Understanding the customer engagement 

----- 1. Which Customer have used the most coupons and what is the total price

select TOP 1 CT.cust_id  , Name ,
count(coupon_id)as no_of_coupons_used , 
...
------ 2. Customer in which cities have the largest number of transactions and what is total

select Top 1
       c.city_id ,cityName , 
	   count(trans_id) as count_of_Transaction ,
...
inner join CustomerTransactionData as CT
on c.Customer_id = CT.Cust_Id
group by c.city_id , cityName 
order by count_of_Transaction desc , Total_sales desc ;



--- Name of customer who had highest purchase in pune
---- (in which citiy have the largest number of transactions and what is total)
...
inner join CustomerTransactionData as CT
on c.Customer_id = CT.Cust_Id
where c.city_id = (select  TOP 1  c.city_id  from customer as c inner join  CustomerTransactionData as CT
on c.customer_id = CT.Cust_Id
inner join city as cd
on c.city_id = cd.City_Id
group by c.city_id ,  cityName
order by  count(trans_id) desc , sum(purchasingAmt) desc) 
group by c.city_id , cityName , Name , customer_id
order by Total_sales desc ;
...
select Top 1 customer_id , Name , sum(PurchasingAmt) as Total_Sales from customerTransactionData as  CT inner join customer as c 
on CT.Cust_Id = c.Customer_id where city_id =  ( select Top 1
       c.city_id 
from city as cd
inner join customer  as c
on cd.City_Id = c.City_id 
...
----- BY CTE (Common Table Expression)

with Most_purchased_city as (
       select Top 1
       c.city_id ,cityName , 
	   count(trans_id) as count_of_Transaction ,
	   round(sum(PurchasingAmt) , 0) as Total_Sales
...
select Top 1 CT.cust_id  , Name as Customer_Name , sum(PurchasingAmt) as Total_sales 
from Most_purchased_city as MC 
inner join Customer as c
...
---- 1.Create view for top 10 customers by purchasing Amount for each sector

GO
create view vw_Top_10_customers as

(
select TOP 10 cust_id , Name as Name_customer , 
sum(purchasingAmt) as Total_purchasing_Amt 
from customerTransactionData  as CT
inner join customer as c
on CT.cust_id = c.customer_id 
group by Cust_id , Name
order by Total_purchasing_Amt desc
);
GO
...
----- TOP 5 customers from house hold and insdustrial sector based on their purchase amount.
Go
create view vw_top_5_customer  as
(
select TOP 5 OrderType , CT.cust_id , Name as Name_customer , 
				   sum(purchasingAmt) as Total_purchasing_Amt
				   from customer as c
				   inner join CustomertransactionData as CT 
				   on c.Customer_id = CT.Cust_Id
				   where orderType = 'Household' or orderType = 'Industrial'
				   group by OrderType , Name , CT.cust_id
				   order by Total_purchasing_Amt desc

);
Go

select * from vw_top_5_customer;
